Feature: User is on Manage VAT Registration and user perform opertations
  ...
  Scenario Outline: Validate Tax Officer can Reject the Application Form 
    ...
    Examples: 
      ...
      | Valid_Manage_VAT_Creation_Reject_01 | user click on reject with comment | Applicant Name | Reject User | You have not provided proper document |
  ...
  Scenario Outline: Validate Tax Officer can post Comment to Tax Payer after Clicking on Additional Clarification /Document Needed 
    ...
    Examples: 
      ...
      | Valid_Manage_VAT_Creation_AdditionalClarification_01 | user enter comment and click on hold | Applicant Name | OnHold User | Your Address is not matching with your passport |
