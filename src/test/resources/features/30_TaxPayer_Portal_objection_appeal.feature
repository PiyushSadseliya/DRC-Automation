Feature: User is on Tax Payer portal Objection and Appeal
  ...
  Scenario Outline: Validate the Description field FileUpload
    ...
    Examples: 
      ...
      | Objection_Appeal_Tax_Payer_20_21_22 | Verify Description | eMar  | btn_Mar_View | 25375123761547615 | Notice raise by you is incorrect | -$%^%$^@$^% | src.zip | File format not found | Big File.xlsx | File size must be less than 2MB | Officers List With Tax Center.xlsx |
