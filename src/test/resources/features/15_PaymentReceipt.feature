...
Feature: Payment Receipt
  ...
  Scenario Outline: Validate the payment Receipt for payment done against particular Month
    Given The user has declared the VAT Liability for the Month of "<month>" usernmame"<uname>"password"<password>"
    When User Clicks on the efiling menu
    When User clicks on view button from pay column for month"<month>"
    Then user must be on Payment Receipt "<bankname>" Details of Taxpayer Name"<name>"Nitva"<Nitva>"Email-id"<emailid>"MobileNo"<mobileno>"Address"<add>" Total Words should be "<twords>"Details of Deposit records "1" Period"<period1>"vat"<vat1>"Interest"<int1>"Penalty"<pen1>"LateFee"<ltfee1>"TotalAmount"<tamount1>"
    ...
    Examples: 
      ...
      | DRC_ATC_PaymentReceipt_01 | Validate the functionality of View button located on the statement of transaction section of e-filing landing page | approveduser@mailinator.com | Test@123 | Advans Bank | Approved User | 20181215035842812 | approveduser@mailinator.com | 1234567890 | Dharamdas ChamberNear Axis Bank,444604, Aketi |         | 800.00 |    0 |    0 |      0 |   800.00 | Eight Hundred | Over the counter | November |
