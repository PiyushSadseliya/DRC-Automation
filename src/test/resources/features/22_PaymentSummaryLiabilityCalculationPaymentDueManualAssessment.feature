...
Feature: Manual Assessment- Payment summary for Liability Calculation and Payment due and Raise notice
  ...
  Scenario Outline: Validate the prev button for no updation in manual assessment when efiling and pymt not done
    ...
    Examples: 
      ...
      | DRC_ATC_Assessment_01 | Validate the prev button,next button for no updation in manual assessment when efiling and pymt not done | laxman.prajapati | admin    | February  | 2019 | Saved Successfully | 500000 |       800,00 |       |  Regression MarchMons|
  ...
  Scenario Outline: Validate the Save,Previous button,Total Tiles,Additionality Liability column for No updation in manual assessment when efiling and pymt not done
    ...
    Examples: 
      ...
      | DRC_ATC_Assessment_02 | Validate the Save,Previous button,prev button,next button Total Tiles,Additionality Liability column | laxman.prajapati | admin    | February  | 2019 | Saved Successfully |  50000 |       310,00 |       |  Regression MarchMons|  0,00 |
  ...
  Scenario Outline: Validate the download functionality of  e-filing schedule Button when e-file schedule file is not uploaded by tax paye
    ...
    Examples: 
      | Test Case ID          | Description                                                                                                                                   | uname            | password | period  | year | msg                | penamt | efiledamount | NITVA             | tpayer            |
      | DRC_ATC_Assessment_03 | Validate the download functionality of  e-filing schedule Button when e-file schedule file is not uploaded by tax payer from tax payer portal | laxman.prajapati | admin    | January | 2019 | Saved Successfully |  50000 |       800,00 | 20181127030648745 |  Regression Marmtue|
  ...
  Scenario Outline: Validate the tax assessment officer is able to raise notice for a particular period for no updation in manual assessment when efiling and pymt not done
    ...
    Examples: 
      ...
      | DRC_ATC_Assessment_04 | Validate the tax assessment officer is able to raise notice for a particular period for no updation in manual assessment when efiling and pymt not done | laxman.prajapati | admin    | February  | 2019 | 500000 |         0,00 |       |  Regression MarchMons|
      | DRC_ATC_Assessment_05 | Validate the tax assessment officer is able to raise notice for a particular period for no updation in manual assessment when efiling and pymt not done | laxman.prajapati | admin    | January  | 2019 | 500000 |         0,00 |       |  Regression MarchMons|
