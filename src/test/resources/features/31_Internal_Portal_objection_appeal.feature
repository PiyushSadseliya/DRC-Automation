Feature: User is on internal Portal for objection and appeal
  ...
  @TC_03_CaseType
  Scenario Outline: Verify the Filter by Functionality based on filter option Case Type Status Priority Tax Officer -internal portal
  ...
  @TC_03_Status
  Scenario Outline: Verify the Filter by Functionality based on filter option Status -internal portal
    Given User is on Internal Portal "<TestcaseID>" "<Description>"
    And User click on case management
    And User click on filter by on case management and click on "<filter>"
  ...
  @TC_08
  Scenario Outline: Validate the priority is set correctly for the individual tax payer. internal portal
    Given User is on Internal Portal "<TestcaseID>" "<Description>"
    And User click on case management
    And User click on filter by on case management and click on "<filter>"
    And User Type id "<id>"
    ...
    Examples: 
      ...
      | Case_Management_ | Verify date filteration functionalityt | drp_CaseID | O18112800000004 |          |      |
  ...
  Scenario Outline: Validate Reject status in the internal portal.
    ...
    Examples: 
      ...
      | Objection_Appeal_internal_portal_15 | Verify Reassign functionality | drp_CaseID_Click | O18091000000003 | Objection updated successfully |
