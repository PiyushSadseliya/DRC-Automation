...
Feature: Update and Appove or Reject the Fuel Rate.
  ...
  Scenario Outline: Validate  Historical Fuel Rate button and Previous button on it.
    ...
    Examples: 
      | TestcaseID                      | Description                                                     |
      | DRC_ATC_FuelManagement_Admin_02 | Validate  Historical Fuel Rate button and Previous button on it |
  ...
  Scenario Outline: Validate the close(X) icon,Effective date functionality on update fuel rate popup
    ...
    Examples: 
      | TestcaseID                      | Description                                                          |
      | DRC_ATC_FuelManagement_Admin_03 | Validate the close(X) icon functionality on update fuel rate popup.. |

  @admin @tc39 @tc40 @tc42 @tc43 @tc44 @tc45 @tc46
  Scenario Outline: Validate the Excise,VAT,Fuel Rate,Royalty,Total are getting calculated,Save on Update fuel rate popup Approval Required Rejected
    Given The officer has logged in "<TestcaseID>""<Description>"
    When the user click on Fuel Rate Management
    Then user is on "Fuel Management"
    Then Contains Header Previous Rate
 ...
 @TC_01
  Scenario Outline: Verify the cancelation of Reject fuel rate functionality - supervisor
    ...
    Examples: 
      ...
      | FX_Management_supervisor_14 | Verify Base Currency | Rates are too high |
  ...
  Scenario Outline: Verify the character limitation on Reject fuel rate    
    ...
    Examples: 
      ...
      | FX_Management_supervisor_14 | Verify Base Currency | Too low | Comment must be of minimum 10 characters. |
  ...
  Scenario Outline: Verify the Reject fuel rate functionality    
    ...
    Examples: 
      | TestcaseID             | Description          | comment            | message                                     |
      | FX_Management_supervisor_14 | Verify Base Currency | Rates are too high | Approve rate has been rejected successfully |
  ...
  @admin @tc39 @tc40 @tc42 @tc43 @tc44 @tc45 @tc46
  Scenario Outline: Validate the Excise,VAT,Fuel Rate,Royalty,Total are getting calculated,Save on Update fuel rate popup Approval Required Approval
    Given The officer has logged in "<TestcaseID>""<Description>"
    When the user click on Fuel Rate Management
    Then user is on "Fuel Management"
    Then Contains Header Previous Rate
